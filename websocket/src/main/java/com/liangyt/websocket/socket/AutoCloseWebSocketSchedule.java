...
public class AutoCloseWebSocketSchedule {
    ...
    public void autoClose() throws Exception {
        ...
        for (String sessionid : onlineList.keySet()) {
            if (!onlineList.containsKey(sessionid)) continue;
            Duration duration = Duration.between(onlineList.get(sessionid).getTime(), now);
            // 没有沟通一分钟及以上的 关闭
            if (duration.toMinutes() >= 1) {
                handler.autoSend(sessionid, "您好久不说话，我要关闭了");
                handler.autoClose(sessionid);
            }
        }
    }
}
